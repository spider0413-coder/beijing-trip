...
// --- Firebase Initialization (Updated with User Config) ---
...
// Use a fixed app ID for path consistency or fallback to default
...
const getBaiduMapLink = (query) => {
  return `https://api.map.baidu.com/place/search?query=${encodeURIComponent(
    query
  )}&region=北京&output=html&src=webapp.beijing_travel`;
};
...
const infoData = {
  souvenirs: [
    {
      name: "稻香村糕點",
      note: "建議買「京八件」禮盒，牛舌餅、棗花酥最經典。",
      tag: "美食",
    },
    {
      name: "吳裕泰花茶",
      note: "茉莉花茶是北京人的最愛，老字號品質有保證。",
      tag: "飲品",
    },
    {
      name: "同仁堂養生品",
      note: "適合送長輩，如阿膠糕或養生茶包。",
      tag: "健康",
    },
    {
      name: "故宮文創",
      note: "故宮內或淘寶旗艦店可買，設計感強，適合年輕人。",
      tag: "文創",
    },
    {
      name: "六必居醬菜",
      note: "明朝老店，甜麵醬與醬八寶是家常送禮首選。",
      tag: "美食",
    },
    {
      name: "瑞蚨祥絲綢",
      note: "百年老字號，絲綢品質極佳，適合定製旗袍或買圍巾。",
      tag: "服飾",
    },
    {
      name: "內聯陞布鞋",
      note: "手工製作千層底布鞋，舒適透氣，長輩最愛。",
      tag: "鞋履",
    },
    {
      name: "御食園果脯",
      note: "北京特產茯苓餅、北京烤鴨零食版，適合分發同事。",
      tag: "零食",
    },
  ],
  tips: [
    ...
    {
      title: "證件與支付",
      text: "台胞證務必隨身攜帶。支付寶/微信支付需提前綁定台灣信用卡或儲值。",
    },
    {
      title: "網路與地圖",
      text: "建議開通漫遊或購買中港卡（免翻牆）。地圖請用「高德地圖」或「百度地圖」，Google Maps 不準。",
    },
    {
      title: "預約制",
      text: "故宮、國家博物館、天安門廣場進入都需要提前7天在微信小程序預約，非常搶手！",
    },
    {
      title: "郵務資訊",
      content: (
        <span>
          ...
          <a
            href={getBaiduMapLink("建國門內大街郵局")}
            target="_blank"
            rel="noreferrer"
            className="text-red-800 underline mx-1 font-bold inline-flex items-center gap-0.5"
          >
            <Map size={14} />
            建國門內大街郵局
          </a>
    ...
    {
      title: "安全與防騙",
      text: "注意「黑車」，堅持打表或用網約車。景區周邊「喝茶」邀約多為騙局。地鐵安檢嚴格，請勿攜帶違禁品。",
    },
    {
      title: "氣候與保養",
      text: "北京氣候極度乾燥，南方人易流鼻血。務必攜帶高保濕乳液、護唇膏，多喝水。",
    },
  ],
  visa: "持有效期限內之「台灣居民來往大陸通行證」（台胞證）即可入境，無需額外簽證。若落地簽需備好身分證、護照、照片及人民幣。",
  clothing:
    "4月初北京氣溫約 8-20°C，日夜溫差大。建議「洋蔥式穿法」：長袖T恤 + 薄毛衣/衛衣 + 防風外套。長城風大，建議帶帽子。鞋子務必穿好走的運動鞋。",
};
  ...
  <div className="flex items-center justify-between bg-white/60 backdrop-blur-sm p-5 rounded-3xl shadow-lg shadow-gray-200/50 border border-white mb-8 animate-in slide-in-from-top-4 duration-700">
    <div className="flex items-center gap-5">
      <div className="w-14 h-14 rounded-full bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center text-gray-600 shadow-inner">
        ...
        {weather.icon === "cloudy" && (
          <CloudSun size={32} className="text-gray-500" />
        )}
        {weather.icon === "rainy" && (
      ...
      <div>
        <div className="text-3xl font-serif font-bold text-gray-800 tracking-tight">
          {weather.temp}
        </div>
    ...
    <div className="text-right">
      {/* 模擬印章風格 */}
      <div className="px-4 py-1.5 border border-red-800/20 text-red-900 text-sm font-serif rounded-sm bg-red-50/50">
        北京
      </div>
    </div>
  </div>
      ...
      <div className="bg-[#fcfcfc] w-full sm:w-[90%] sm:max-w-md h-[90vh] sm:h-auto sm:max-h-[85vh] rounded-t-[2.5rem] sm:rounded-[2rem] flex flex-col shadow-2xl relative overflow-hidden">
        ...
        <div className="pt-14 px-8 pb-6 relative shrink-0">
          ...
          <div className="animate-in slide-in-from-bottom-2 duration-500 delay-100">
            <h2 className="text-5xl font-serif font-bold text-gray-900 tracking-wide mb-4 leading-tight">
              {item.title}
            </h2>
            {/* Ink Stroke Divider */}
        ...
        <div className="px-8 pb-10 overflow-y-auto no-scrollbar">
          ...
          <div className="flex gap-4 mb-8">
            {item.mapQuery && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex-1 bg-gray-100 text-gray-700 py-4 rounded-2xl flex items-center justify-center gap-2 font-bold text-base hover:bg-gray-200 transition-colors"
              >
                <MapPin size={20} /> 查看地圖
              </a>
            )}
            {item.ticketUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex-1 bg-red-800 text-white py-4 rounded-2xl flex items-center justify-center gap-2 font-bold text-base hover:bg-red-900 transition-colors"
              >
          ...
          <div className="space-y-10">
            <div
              className="text-xl text-gray-700 leading-9 guide-content text-justify font-light"
            ...
            {item.photoSpots && (
              <div className="bg-white p-8 rounded-3xl shadow-lg shadow-gray-100/80 border border-gray-50">
                <h3 className="font-serif font-bold text-2xl text-gray-900 mb-6 flex items-center gap-3">
                ...
                <div className="grid gap-4">
                  {item.photoSpots.map((spot, idx) => (
                    <div key={idx} className="flex items-start gap-4 p-2">
                      <span className="font-serif text-3xl font-bold text-gray-200 -mt-2">
                        0{idx + 1}
                      </span>
                      <span className="text-lg text-gray-800 font-medium pt-1">
                        {spot}
                      </span>
                    </div>
                  ))}
    ...
    <div className="p-8 pb-4 relative z-10">
      <div className="flex justify-between items-start mb-8">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold tracking-wide text-gray-600 text-base">
            {flight.airline}
          </span>
        </div>
        <div className="flex flex-col items-end">
          <span className="font-serif font-bold text-red-900 text-2xl">
            {flight.code}
          </span>
          <span className="text-xs text-gray-400 uppercase tracking-widest mt-1">
            {flight.type === "outbound" ? "Outbound" : "Inbound"}
          </span>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex flex-col items-center flex-1 px-8">
          ...
          <div className="text-xs text-gray-400 mt-2 font-serif italic">
    ...
    <div className="p-8 pt-4 bg-gray-50/50">
      <div className="grid grid-cols-3 gap-6 mb-2">
        <div>
          ...
          <div className="font-bold text-gray-800 text-xl font-serif">
            {flight.date}
          </div>
        </div>
    ...
    <div className="relative z-10">
      <div className="flex items-start justify-between mb-6">
        <div className="flex-1 pr-4">
          <span className="inline-block px-3 py-1.5 bg-gray-100 text-gray-600 text-xs font-bold rounded mb-3 tracking-wide uppercase">
            Hotel Voucher
          </span>
          <h3 className="text-3xl font-serif font-bold text-gray-900 leading-tight mb-3">
            {hotel.name}
          </h3>
          <a
          ...
          >
            ...
            <span className="text-base font-light border-b border-dashed border-gray-300 pb-0.5 group-hover/link:border-red-800">
              {hotel.address}
            </span>
          </a>
        </div>
      ...
      <div className="grid grid-cols-2 gap-6 mt-8">
        <div className="p-5 bg-gray-50 rounded-2xl border border-gray-100 flex flex-col items-center justify-center text-center">
          ...
          <div className="font-serif font-bold text-gray-900 text-2xl">
            {hotel.checkIn.slice(5)}
          </div>
          <div className="text-sm text-gray-500 mt-1">15:00 後</div>
        </div>
        ...
        <div className="p-5 bg-gray-50 rounded-2xl border border-gray-100 flex flex-col items-center justify-center text-center">
          ...
          <div className="font-serif font-bold text-gray-900 text-2xl">
            {hotel.checkOut.slice(5)}
          </div>
          <div className="text-sm text-gray-500 mt-1">12:00 前</div>
        </div>
      </div>
      ...
      <div className="mt-8 flex items-center justify-between border-t border-gray-100 pt-6">
        ...
        <div className="text-base text-gray-400 font-serif italic">
...
const BookingTab = () => (
  <div className="p-6 space-y-4 pb-32 animate-in fade-in duration-500">
    ...
    <div className="mb-8 flex items-baseline gap-3">
      <h2 className="text-4xl font-serif font-bold text-gray-900">行程預訂</h2>
      <span className="text-sm text-gray-400 tracking-widest uppercase">
        Bookings
      </span>
    </div>
    {bookingData.flights.map((flight, idx) => (
      <BoardingPass key={idx} flight={flight} />
    ))}
    <div className="h-6"></div> {/* Spacer */}
    <HotelCard hotel={bookingData.hotel} />
  </div>
);
...
const DiningTab = () => (
  <div className="p-6 pb-32 animate-in fade-in duration-500">
    <div className="mb-8 flex items-baseline gap-3">
      <h2 className="text-4xl font-serif font-bold text-gray-900">尋味北京</h2>
      <span className="text-sm text-gray-400 tracking-widest uppercase">
        Gastronomy
      </span>
    </div>
    ...
    <div className="space-y-5">
      {diningData.map((item) => (
        <div
          key={item.id}
          className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-4 group hover:shadow-md transition-shadow"
        >
          ...
          <p className="text-base text-gray-600 leading-relaxed pt-3 border-t border-dashed border-gray-100">
            {item.note}
          </p>
        </div>
      ))}
...
const ExpenseTracker = () => {
  ...
  useEffect(() => {
    ...
    // RULE 1 & 2: Use strict path and simple query (no complex orderBy if combined with other filters, but here it's fine as we just want list)
    // We will do sorting in memory just to be absolutely safe with indexes.
  ...
  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-gray-400 gap-2">
        ...
        <span className="text-sm font-serif">雲端資料同步中...</span>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-8 pb-32 animate-in fade-in duration-500">
      ...
      <div className="bg-gray-900 text-white p-8 rounded-[2rem] shadow-xl shadow-gray-300 relative overflow-hidden">
        <div className="relative z-10">
          <div className="text-gray-400 text-base font-medium mb-3 tracking-widest uppercase">
            Total Expenses
          </div>
          <div className="text-6xl font-serif font-bold flex items-baseline gap-4 mb-3">
            <span className="text-3xl font-sans text-gray-400">¥</span>{" "}
            {totalCNY.toLocaleString()}
          </div>
          {totalTWD > 0 && (
            <div className="text-xl font-medium text-gray-400 flex items-baseline gap-2">
      ...
      <div className="space-y-6">
        <h3 className="font-serif text-xl font-bold text-gray-900 ml-2 flex items-center gap-2">
          {editingId ? (
            <>
              <Edit2 size={20} /> 編輯消費
            </>
        ...
        <div
          className={`bg-white p-6 rounded-3xl shadow-lg shadow-gray-100 border transition-colors flex flex-col gap-4 ${
            editingId ? "border-red-100 bg-red-50/20" : "border-white"
          }`}
        >
          {/* Row 1: Date & Currency */}
          <div className="flex gap-4">
            <div className="flex-1 relative">
              <select
                className="w-full bg-gray-50 border-0 rounded-xl px-5 py-4 text-gray-800 text-base focus:bg-white transition-all outline-none appearance-none font-medium"
                value={selectedDay}
                onChange={(e) => setSelectedDay(e.target.value)}
              >
                {itineraryData.map((d) => (
                  <option key={d.day} value={`Day ${d.day}`}>
            ...
            <div className="w-28 relative">
              <select
                className="w-full bg-gray-50 border-0 rounded-xl px-4 py-4 text-base font-bold text-gray-600 outline-none appearance-none text-center"
                value={currency}
          ...
          <input
            ...
            className="w-full bg-gray-50 border-0 rounded-xl px-5 py-4 text-gray-800 text-base focus:bg-white transition-all outline-none"
            value={newTitle}
          ...
          <div className="flex gap-4">
            <input
              type="number"
              placeholder="金額"
              className="flex-1 bg-gray-50 border-0 rounded-xl px-5 py-4 text-gray-800 text-base focus:bg-white transition-all outline-none font-medium"
              value={newAmount}
            ...
            {editingId && (
              <button
                onClick={resetForm}
                className="bg-gray-200 text-gray-600 px-5 rounded-xl font-bold text-base hover:bg-gray-300 transition-colors flex items-center justify-center"
              >
                <RotateCcw size={22} />
              </button>
            )}
            <button
              ...
              } text-white px-8 rounded-xl font-bold text-base transition-colors flex items-center justify-center min-w-[3.5rem] shadow-md`}
            >
      ...
      <div className="space-y-5">
        {expenses.map((item) => (
          <div
            key={item.id}
            className={`flex justify-between items-center bg-white/60 p-6 rounded-2xl border transition-all ${
              editingId === item.id
          ...
          >
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                <div className="text-sm text-gray-400 mt-1 font-medium bg-gray-100 px-2.5 py-0.5 rounded-full inline-block">
                  {item.day}
                </div>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="font-serif font-bold text-gray-900 text-2xl mr-2">
...
const InfoTab = () => (
  <div className="p-6 space-y-10 pb-36 animate-in fade-in duration-500">
    ...
    <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-gray-200/50 border border-white relative overflow-hidden group">
      <div className="relative z-10">
        <div className="flex items-center gap-4 mb-5">
          <span className="p-3 bg-gray-100 rounded-full text-gray-600">
            <Wind size={24} />
          </span>
    ...
    {/* Tips */}
    <div className="space-y-5">
      <h3 className="font-serif font-bold text-2xl text-gray-900 ml-2">
        旅遊須知
      </h3>
      {infoData.tips.map((tip, idx) => (
        <div
          key={idx}
          className="bg-white/80 backdrop-blur-sm p-7 rounded-3xl border border-gray-100 hover:border-gray-200 transition-colors"
        >
          <div className="flex items-center gap-3 mb-3">
            <div
              className={`w-2 h-2 rounded-full ${
                tip.title.includes("安全") || tip.title.includes("入境")
                  ? "bg-orange-500"
                  : "bg-red-800"
              }`}
            ></div>
            <div className="font-bold text-gray-800 text-lg flex items-center gap-2">
              {tip.title}
              {tip.title.includes("郵務") && (
                <Mail size={18} className="text-gray-400" />
              )}
              {(tip.title.includes("安全") || tip.title.includes("入境")) && (
                <AlertTriangle size={18} className="text-orange-500" />
              )}
              {tip.title.includes("入境") && (
                <Briefcase size={18} className="text-gray-400" />
              )}
            </div>
          </div>
          <div className="text-base text-gray-600 leading-loose pl-5">
    ...
    {/* Souvenirs */}
    <div>
      <h3 className="font-serif font-bold text-2xl text-gray-900 mb-6 ml-2">
        必買伴手禮
      </h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        {infoData.souvenirs.map((item, idx) => (
          <div
            key={idx}
            className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col h-full hover:-translate-y-1 transition-transform duration-300"
          >
            <span className="text-xs font-bold text-gray-400 tracking-wider mb-3 uppercase">
              {item.tag}
            </span>
            <div className="font-serif font-bold text-gray-900 mb-2 text-xl">
              {item.name}
            </div>
            <div className="text-base text-gray-500 leading-relaxed mt-auto">
              {item.note}
            </div>
          </div>
        ))}
...
export default function BeijingTravelApp() {
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] font-sans text-gray-900 max-w-md mx-auto shadow-2xl relative selection:bg-red-100 selection:text-red-900 overflow-hidden text-base">
      {/* Subtle Ink Background Effect */}
      <div
        className="fixed inset-0 pointer-events-none z-0 opacity-40 mix-blend-multiply"
        style={{
          backgroundImage:
            "radial-gradient(circle at 10% 20%, #f3f4f6 0%, transparent 40%), radial-gradient(circle at 90% 80%, #f3f4f6 0%, transparent 40%)",
        }}
      ></div>

      {/* Header - Clean Ink Style */}
      <header className="pt-16 pb-4 px-8 bg-transparent relative z-20">
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-5xl font-serif font-bold text-gray-900 tracking-wide leading-none">
              北京
              <span className="text-red-800 text-2xl align-top ml-1">●</span>
            </h1>
            <p className="text-sm text-gray-400 font-medium tracking-[0.2em] mt-3 uppercase">
              Travel Guide
            </p>
          </div>
          {/* Logo Mark */}
          <div className="w-14 h-14 border-2 border-gray-900 rounded-xl flex items-center justify-center rotate-3 bg-white/50 backdrop-blur-sm">
            <span className="font-serif font-bold text-gray-900 text-3xl">
              京
            </span>
          </div>
        </div>
      ...
      {/* Main Content Area */}
      <main className="h-[calc(100vh-110px)] overflow-y-auto no-scrollbar relative z-10">
        {/* Render Tabs */}
        {activeTab === "itinerary" && (
          <div className="animate-in fade-in duration-500">
            ...
            <div className="flex overflow-x-auto gap-4 px-8 py-6 no-scrollbar snap-x sticky top-0 z-30 bg-gradient-to-b from-[#fcfcfc] via-[#fcfcfc] to-transparent">
              {itineraryData.map((d) => (
                ...
                >
                  <span className="text-base font-bold whitespace-nowrap font-serif">
                    Day {d.day}
                  </span>
                </button>
              ))}
            ...
            <div className="px-8 pb-32">
              <div className="mb-8">
                ...
                <div className="text-base text-gray-400 font-medium tracking-wide">
                  {currentDayData.date}
                </div>
              </div>
                ...
                {currentDayData.items.map((item, index) => {
                  ...
                  return (
                    <div key={index} className="relative pl-14 group">
                      {/* Timeline Node */}
                      <div
                        className={`absolute left-0 top-1.5 w-12 h-12 rounded-full z-10 flex items-center justify-center transition-all duration-300 border-4 border-[#fcfcfc] ${
                          isAttraction
                      ...
                      >
                        <div className="flex justify-between items-start mb-3">
                          <span
                            className={`text-xs font-bold tracking-widest font-mono uppercase ${
                              isAttraction ? "text-gray-400" : "text-gray-300"
                            }`}
                          >
                            {item.time}
                          </span>
                        ...
                        {item.mapQuery && !isAttraction && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="mt-3 text-sm text-red-800 flex items-center gap-1.5 font-bold"
                          >
                            <MapPin size={14} /> 導航
                          </a>
                        )}
                        ...
                        {isAttraction && item.photoSpots && (
                          <div className="mt-5 pt-5 border-t border-gray-50 flex gap-3 overflow-hidden">
                            {item.photoSpots.slice(0, 2).map((spot, i) => (
                              <span
                                key={i}
                                className="text-xs bg-gray-50 text-gray-600 px-4 py-2 rounded-full font-medium"
                              >
                                {spot}
                              </span>
                            ))}
      ...
      <nav className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur-xl rounded-full shadow-2xl shadow-gray-200/50 border border-white/50 flex items-center z-40 px-8 py-5 gap-8">
        ...
        >
          ...
          />
          {activeTab === "itinerary" && (
            <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-red-800 rounded-full"></div>
          )}
        </button>
        <button
